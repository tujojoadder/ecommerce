...
class ProductController extends Controller
{
    ...
    public function store(ProductRequest $request)
    {
        try {
            ...
            $data->shipping_in_dhaka = $request->shipping_in_dhaka;
            $data->shipping_out_dhaka = $request->shipping_out_dhaka;
            ...
            if ($request->hasFile('subimage')) {
                foreach ($request->file('subimage') as $subimage) {
                    ...
                    $subFile->folder('products/sub')->prefix('product_subimage')->upload($subimage) ? $subimageName = $subFile->getName() : null;
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            // === Sub Images Update ===
            if ($request->hasFile('subimage')) {
                // পুরানো সাব-ইমেজ ডিলিট করে আবার insert করব
                $oldImages = $product->images; // relation ধরলাম: Product::hasMany(ProductImage::class, 'product_id')
                ...
                // নতুন সাব-ইমেজ insert
                /*                 foreach ($request->file('subimage') as $file) {
                    $path = $file->store('products/sub', 'public');
